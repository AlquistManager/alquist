...
class MessageText(State):
    def execute(self, request_data) -> dict:
        ...
        loggers.get(self.bot).get("state_logger").debug('Response: ' + text, extra={'uid': request_data.get('session', False)})
...
class MessageButtons(State):
    def execute(self, request_data) -> dict:
        ...
        for butt in butts:
            ...
            loggers.get(self.bot).get("state_logger").debug('Button: ' + str(new_butt), extra={'uid': request_data.get('session', False)})
...
class MessageCheckboxes(State):
    def execute(self, request_data) -> dict:
        ...
        for checkbox in checkboxes:
            ...
            loggers.get(self.bot).get("state_logger").debug('Checkbox: ' + str(new_checkbox), extra={'uid': request_data.get('session', False)})
...
class MessageIframe(State):
    def execute(self, request_data) -> dict:
        ...
        loggers.get(self.bot).get("state_logger").debug('Iframe: ' + str(url), extra={'uid': request_data.get('session', False)})
        if old_response:
            old_response.append(message)
        else:
            old_response = [message]
